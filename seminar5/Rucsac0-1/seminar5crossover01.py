...
def aritmetica_simpla(p1, p2, alpha):
    c1 = p1.copy()
    c2 = p2.copy()
    n = len(p1)
    i = np.random.randint(1, n) # se alege un punct de taiere fara a fi primul element
    for poz in range(i, n):
        c1[poz] = p1[poz] * alpha + p2[poz] * (1 - alpha)
        c2[poz] = p1[poz] * (1 - alpha) + p2[poz] * alpha
    return c1, c2
...
def crossover_populatie(pop,cal,c,v,max,pc,alpha): #alpha e ponderea
    ...
    for i in range ( 0, dim -1, 2): #parcurgem populatia din 2 in 2
        ...
        if r<= pc:
            c1,c2 = aritmetica_simpla(p1, p2, alpha) #c1 si c2 sunt copiii obtinuti prin crossover
            f1,v1 = ok(c1,c,v,max)
