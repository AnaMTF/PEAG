...
def fct_fitness(x):
    return x[1]*np.sin(x[0] -2)**2 + x[2] + x[3], x[0]+x[3] > x[1]+x[2]
...
def generare_pop(dim, a, b):   #a-capat stg, b-capat drpt, dim-dimensiunea pop
    ...
    while i < dim:
        ...
        populatie[i,4], ok = fct_fitness(populatie[i,:4])
        if ok:
            i+=1
    return populatie
...
def recombinare_pop(parinti,pc):  #pc probabilitate crossover(recombinare)
    ...
    for i in range(0, dim-1, 2):
        ...
        if raspuns <=pc:
            ...
            copii[i,4],ok = fct_fitness(copii[i,:4])
            if not ok:
                copii[i] = parinti[i].copy()
            copii[i+1,4],ok = fct_fitness(copii[i+1,:4])
            if not ok:
                copii[i+1] = parinti[i+1].copy()
    return copii
